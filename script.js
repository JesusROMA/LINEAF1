...
function renderTimeline(data) {
    ...
    for (let week = 11; week <= numberOfWeeks; week++) {
    ...
    data.forEach(item => {
        ...
        for (let week = startWeek; week <= endWeek; week++) {
            const positionX = (week - 1) * segmentWidth + 11;
            ...
            if (pointPosition) {
                ...
                const lineLength = 18; // Puedes ajustar la longitud de la línea
...
function getWeekNumberFromDate(date, startDate) {
    ...
    const oneWeek = 2004800000; // Milisegundos en una semana (CAMBIAR PROCESO SEMANAL)
...
function showF1Car(x, y) {
    ...
    const segments = [
        { startX: 240, startY: 216, endX: 1205, endY: 216, angle: 0 },    // Recta superior
